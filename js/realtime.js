...
// Check for new activities and data changes periodically
function checkForNewActivities() {
    // Reload all data from localStorage to get updates from other tabs/devices
    ...
    // Check if data has changed (new items, requests, or transactions)
    ...
    // If we're on a relevant section and data changed, refresh the display
    if (dataChanged) {
    ...
    const newActivities = activityLog.filter(activity => {
        ...
        return activityTime > lastActivityCheck && activity.user !== currentUser.displayName;
    });
