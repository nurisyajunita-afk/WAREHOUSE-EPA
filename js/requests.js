...
async function addNewRequest(event) {
    ...
    try {
        ...
        alert('Gagal membuat permintaan: ' + e.message);
    }
}

async function approveRequest(requestId, level) {
    const request = requests.find(r => r.id === requestId);
    if (request) {
        try {
            if (level === 'staff' && currentUser.permissions.approveStaffRequests) {
                await updateRequestStatusOnServer(request.id, 'Menunggu Manager');
                ...
                await updateRequestStatusOnServer(request.id, 'Disetujui');
            ...
            alert('Gagal memperbarui status permintaan: ' + e.message);
        }
    }
}
        ...
        if (request) {
            try {
                if ((request.status === 'Menunggu Staff' && currentUser.permissions.approveStaffRequests) ||
                    ...
                    await updateRequestStatusOnServer(request.id, 'Ditolak');
                ...
                alert('Gagal menolak permintaan: ' + e.message);
            }
        }
    }
}
